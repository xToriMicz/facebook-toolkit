...
import { Env, getSessionFromReq, getUserPageId, getUserPageToken, setUserPage, getDecryptedPageToken } from "../helpers";
...
media.post("/reels", async (c) => {
  ...
  try {
    ...
    const uploadRes = await fetch(`https://rupload.facebook.com/video-upload/v25.0/${videoId}`, {
      method: "POST", headers: { "Authorization": `OAuth ${pageToken}`, "file_url": video_url },
    });
    ...
    const pubRes = await fetch(`https://graph.facebook.com/v25.0/${targetPageId}/video_reels`, {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ video_id: videoId, upload_phase: "finish", video_state: "PUBLISHED", description: description || "", access_token: pageToken }),
    });
    const pubData: any = await pubRes.json();
    if (pubData.error) return c.json({ error: pubData.error.message, step: "publish" }, 400);
    await c.env.DB.prepare(
      "INSERT INTO posts (message, fb_post_id, user_fb_id, status, created_at) VALUES (?, ?, ?, 'posted', ?)"
    ).bind("[Reel] " + (description || ""), pubData.id || videoId, session.fb_id, new Date().toISOString()).run();
...
media.post("/stories", async (c) => {
  ...
  try {
    ...
    await c.env.DB.prepare(
      "INSERT INTO posts (message, fb_post_id, user_fb_id, status, created_at) VALUES (?, ?, ?, 'posted', ?)"
    ).bind("[Story] " + (isVideo ? "Video" : "Photo"), data.id || data.post_id, session.fb_id, new Date().toISOString()).run();
...
media.post("/settings", async (c) => {
  ...
  const encKey = c.env.TOKEN_ENCRYPTION_KEY || c.env.FB_APP_SECRET;
  if (page_id && page_token) await setUserPage(c.env.KV, session.fb_id, page_id, page_token, page_name || "", encKey);
