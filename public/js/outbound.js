...
import { toast as d, insEsc as s, showNotify as p } from "./utils.js";
var v = {
...
export async function addTargetPage() {
  var e = document.getElementById("outTargetInput"),
    a = (e.value || "").trim();
  if (!a) {
    d(
      "err",
  ...
  var u = document.getElementById("outTargetStatus");
  ((u.textContent =
    "\u23F3 \u0E01\u0E33\u0E25\u0E31\u0E07\u0E04\u0E49\u0E19\u0E2B\u0E32\u0E40\u0E1E\u0E08..."),
    (u.className = "toast"));
  try {
    var r = await fetch("/api/outbound/resolve", {
        method: "POST",
    ...
    if (!t.id) {
      ((u.textContent =
        t.error ||
        "\u0E44\u0E21\u0E48\u0E1E\u0E1A\u0E40\u0E1E\u0E08\u0E19\u0E35\u0E49"),
        (u.className = "toast err"));
      return;
    }
    u.textContent = "\u23F3 \u0E40\u0E1E\u0E34\u0E48\u0E21 " + t.name + "...";
    var E = await fetch("/api/outbound/targets", {
        method: "POST",
        credentials: "same-origin",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          page_id: n.selectedPage.id,
          target_page_id: t.id,
          target_page_name: t.name,
          target_page_url: a,
        }),
      }),
      i = await E.json();
    ...
    ((u.textContent = "Error: " + o.message), (u.className = "toast err"));
  }
}
export async function loadTargetPages() {
  var e = document.getElementById("outTargetList");
  if (!n.selectedPage) {
    e.innerHTML =
  ...
  try {
    var a = await fetch("/api/outbound/targets?page_id=" + n.selectedPage.id, {
        credentials: "same-origin",
      }),
      u = await a.json(),
      r = u.targets || [];
    ...
    e.innerHTML = r
      .map(function (t) {
        var E = t.comment_tone || "casual",
          i = t.enabled === 1 || t.enabled === !0;
        return (
          ...
          s(t.target_page_name || t.target_page_id) +
          '</div><div style="font-size:0.65rem;color:var(--text-muted);margin-top:1px">' +
          (i
            ? "\u{1F7E2} " + ({"all":"\u0E17\u0E38\u0E01\u0E42\u0E1E\u0E2A","random":"\u0E1A\u0E32\u0E07\u0E42\u0E1E\u0E2A","one":"\u0E42\u0E1E\u0E2A\u0E40\u0E14\u0E35\u0E22\u0E27/\u0E27\u0E31\u0E19"}[t.reply_mode||"all"] || "\u0E17\u0E38\u0E01\u0E42\u0E1E\u0E2A")
            : "\u26AB \u0E1B\u0E34\u0E14\u0E2D\u0E22\u0E39\u0E48") +
          (t.last_commented_at
            ? " \xB7 \u0E25\u0E48\u0E32\u0E2A\u0E38\u0E14: " +
              new Date(t.last_commented_at).toLocaleDateString("th-TH", {
                day: "numeric",
                month: "short",
              })
            : "") +
          ...
          (i ? "rgba(34,197,94,0.3)" : "rgba(255,255,255,0.1)") +
          ...
          (i
            ? '<div style="margin-top:6px;display:flex;gap:6px;flex-wrap:wrap"><select onchange="updateTargetMode(' +
              ...
              [["all","\u0E15\u0E2D\u0E1A\u0E17\u0E38\u0E01\u0E42\u0E1E\u0E2A"],["random","\u0E15\u0E2D\u0E1A\u0E1A\u0E32\u0E07\u0E42\u0E1E\u0E2A"],["one","\u0E42\u0E1E\u0E2A\u0E40\u0E14\u0E35\u0E22\u0E27/\u0E27\u0E31\u0E19"]]
                .map(function (o) {
                  return (
                    '<option value="' +
                    o[0] +
                    '"' +
                    (o[0] === (t.reply_mode || "all") ? " selected" : "") +
                    ">" +
                    o[1] +
                    "</option>"
                  );
...
export async function toggleTargetPage(e, a) {
  try {
    (await fetch("/api/outbound/targets/" + e, {
      method: "PUT",
      credentials: "same-origin",
      headers: { "Content-Type": "application/json" },
...
export async function removeTargetPage(e) {
  try {
    (await fetch("/api/outbound/targets/" + e, {
      method: "DELETE",
      credentials: "same-origin",
...
export async function updateTargetTone(e, a) {
  try {
    await fetch("/api/outbound/targets/" + e, {
      method: "PUT",
      credentials: "same-origin",
...
export async function updateTargetMode(e, a) {
  try {
    await fetch("/api/outbound/targets/" + e, {
      method: "PUT",
      credentials: "same-origin",
...
export async function updateTargetMaxDay(e, a) {
  try {
    await fetch("/api/outbound/targets/" + e, {
      method: "PUT",
      credentials: "same-origin",
...
export async function loadOutboundQueue() {
  var e = document.getElementById("outApprovalQueue");
  if (!n.selectedPage) {
    e.innerHTML =
      '<div class="empty-state">\u0E01\u0E23\u0E38\u0E13\u0E32\u0E40\u0E25\u0E37\u0E2D\u0E01\u0E40\u0E1E\u0E08\u0E01\u0E48\u0E2D\u0E19</div>';
    return;
  ...
  try {
    var a = await fetch(
        "/api/outbound/queue?page_id=" + n.selectedPage.id + "&status=pending",
        { credentials: "same-origin" },
      ),
      u = await a.json(),
      r = u.comments || [];
    ...
    e.innerHTML = r
      .map(function (t) {
        var E = new Date(t.created_at).toLocaleString("th-TH", {
          day: "numeric",
          month: "short",
          hour: "2-digit",
          minute: "2-digit",
...
export async function approveOutbound(e) {
  var a = document.getElementById("outEdit_" + e),
    u = a ? a.value.trim() : "";
  try {
    var r = await fetch("/api/outbound/queue/" + e + "/approve", {
        method: "POST",
        credentials: "same-origin",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ comment_text: u || void 0 }),
      }),
      t = await r.json();
    t.ok
      ? (p(
...
export async function rejectOutbound(e) {
  try {
    (await fetch("/api/outbound/queue/" + e + "/reject", {
      method: "POST",
      credentials: "same-origin",
    }),
      p(
...
export async function loadOutboundHistory() {
  var e = document.getElementById("outHistoryList");
  if (!n.selectedPage) {
    e.innerHTML =
      '<div class="empty-state">\u0E01\u0E23\u0E38\u0E13\u0E32\u0E40\u0E25\u0E37\u0E2D\u0E01\u0E40\u0E1E\u0E08\u0E01\u0E48\u0E2D\u0E19</div>';
    return;
  ...
  try {
    var a = await fetch("/api/outbound/history?page_id=" + n.selectedPage.id, {
        credentials: "same-origin",
      }),
      u = await a.json(),
      r = u.comments || [];
    ...
    e.innerHTML = r
      .map(function (t) {
        var E = new Date(t.created_at).toLocaleString("th-TH", {
          ...
          i = l[t.status] || "var(--text-muted)",
          o = c[t.status] || t.status;
        return (
